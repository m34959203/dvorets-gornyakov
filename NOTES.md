...
- **#24 Фолбэк-фото на главной/каталоге + about + hero** (по браузер-проверке Cowork `docs/handoffs/2026-05-21-1.md`). B1: главная (`PosterCard`) и каталог (`EventsCatalog`) рисовали обложку только градиентом — поле фото не пробрасывалось. Добавлено `image?` в `Poster`, обложка = фото под полупрозрачным тинтом (фолбэк на градиент, если фото нет). Маппинг вынесен в общий `src/lib/event-image.ts` (`eventImage`/`clubImage`), на него переведены `EventCard`/деталка/`ClubCard` (убраны 3 дубля). B2: на `/about` добавлено фото фасада (dvorets-01) + подпись-атрибуция. B3: в `EtnoHero` под орнамент добавлен размытый фасад (blur 6px, opacity .3) — фотографическая фактура вместо «скелета». `tsc`+`build` чисто, задеплоено. **Закрывает B1/B2/B3 из проверки Cowork.** Директора (Ширекбаева, dvorets-02) НЕ ставил — карточка руководства про Дильдебаева (тёзка дворца), нужна отдельная запись.
